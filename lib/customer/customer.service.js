...
Customer.create = (newCustomer, result) => {
  sql.query("INSERT INTO ecommerce.customer SET ?", newCustomer, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
const get_customer_by_id = (Customer) => (cust_id) => {
  return customer.findOne({ cust_id: cust_id });
};

const get_customer_by_name = (Customer) => (name) => {
  return customer.filter.includes({ name: name });
};
